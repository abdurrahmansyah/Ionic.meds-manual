...
@Component({
  selector: 'app-penunjang-detail',
  templateUrl: './penunjang-detail.page.html',
  styleUrls: ['./penunjang-detail.page.scss'],
})
export class PenunjangDetailPage implements OnInit {
  datas: SubCategory[] = [];
  ...
  IsText(type: string) {
    if (type == dataTemp.type.text) return true;
    else return false;
  }

  IsSub(type: string) {
    if (type == dataTemp.type.sub) return true;
    else return false;
  }

  IsSubSub(type: string) {
    if (type == dataTemp.type.subsub) return true;
    else return false;
  }

  IsImg(type: string) {
    if (type == dataTemp.type.img) return true;
    else return false;
  }

  IsRef(type: string) {
    if (type == dataTemp.type.ref) return true;
    else return false;
  }

  IsBtn(type: string) {
    if (type == dataTemp.type.btn) return true;
    else return false;
  }

  IsAudio(type: string) {
    if (type == dataTemp.type.audio) return true;
    else return false;
  }
  ...
  BtnThru(data: SubCategory) { // PERLU EDIT
    try {
      const newData: Category = { id: 0, data: data.data, title: data.title! };
  ...
  GetTabParent(tab: string): string {
    ...
    if (tab == dataTemp.bagian.asesmen) return dataTemp.route.asesmen;
  ...
  GetTabTo(tab: string): string {
    ...
    if (tab == dataTemp.bagian.asesmen) return dataTemp.route.detail;
